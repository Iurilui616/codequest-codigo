...
const SFX = {
    ...
    play(n) {
        const fx = {
            jump: () => this.tone(330, 0.1, 'square', [440, 550], 0.1),
            bump: () => this.tone(150, 0.1, 'square', [100], 0.1),
            coin: () => { this.tone(988, 0.08, 'square', [], 0.1); setTimeout(() => this.tone(1319, 0.3, 'square', [], 0.1), 80) },
            stamp: () => this.tone(200, 0.1, 'sawtooth', [100, 50], 0.15),
            die: () => { [400, 350, 300, 250, 200, 150].forEach((f, i) => setTimeout(() => this.tone(f, 0.1, 'triangle', [], 0.2), i * 100)); },
            flag: () => { [440, 554, 659, 880, 440, 554, 659, 880].forEach((f, i) => setTimeout(() => this.tone(f, 0.15, 'square', [], 0.1), i * 120)); }
        };
        if (fx[n]) fx[n]();
    }
};
...
const Generators = {
    ...
    1: (map, cols, rows, entities) => {
        ...
        for (let x = 0; x < cols; x++) {
            ...
            // Blocks
            if (x === 15) { map[x][gh - 4] = TILES.BLOCK; }
            if (x > 20 && x < 26 && x % 2 !== 0) { map[x][gh - 4] = TILES.BRICK; map[x][gh - 8] = TILES.BLOCK; }
    ...
    3: (map, cols, rows, entities) => {
        ...
        for (let x = 0; x < cols; x++) {
            if (x < 10) { for (let y = gh; y < rows; y++) map[x][y] = TILES.GROUND; }
            // Floating platforms
    ...
    emit(x, y, c) {
        ...
        for (let i = 0; i < n; i++) {
            this.list.push({
                ...
                life: c.life || 30, maxLife: c.life || 30,
                color: c.color || '#fff',
                size: c.size || 4
            })
        }
...
class Game {
    constructor() {
        ...
        this.player = {
            x: 100, y: 100, w: 24, h: 30,
            vx: 0, vy: 0,
            grounded: false,
            facing: 1,
            coins: 0,
            hp: 1, // Mario style (small/big) - kept simple for now
            invincible: 0
        };
    ...
    bindUI() {
        // Menu buttons
        const ids = ['btn-start-1', 'btn-start-2', 'btn-start-3', 'btn-start-4'];
        ids.forEach(id => {
        ...
        window.addEventListener('keydown', e => {
            ...
            if (e.code === 'Space' || e.code === 'ArrowUp' || e.code === 'KeyW') {
                if (this.player.grounded && this.state === 'playing') {
                    ...
                    SFX.play('jump');
                    this.particles.emit(this.player.x + 12, this.player.y + 32, { color: '#fff', count: 4, size: 2 });
                }
            }
        });
    ...
    loadLevel(lvl) {
        ...
        for (let y = this.rows - 1; y > 0; y--) {
            if (this.map[endX][y] === TILES.GROUND || this.map[endX][y] === TILES.BRICK) { endY = y; break; }
        }
        // Pole
        ...
        this.entities.forEach(e => {
            ...
            if (P.x < e.x + e.w && P.x + P.w > e.x && P.y < e.y + e.h && P.y + P.h > e.y) {
                if (P.vy > 0 && P.y + P.h < e.y + e.h / 2 + 10) {
                    ...
                    P.vy = -8;
                    SFX.play('stamp');
                    this.particles.emit(e.x, e.y, { color: '#800', count: 5 });
                } else {
                    this.die();
                }
            }
    ...
    getTheme() {
        const themes = {
            1: { sky: '#0a0a1a', ground: '#1a3a2a', brick: '#2a5a4a', accent: '#00ffa8', enemy: '#ff6b6b' },
            2: { sky: '#0f0520', ground: '#2a1a4a', brick: '#4a2a6a', accent: '#bf7fff', enemy: '#ff9f43' },
            3: { sky: '#000510', ground: '#0a2040', brick: '#1a3060', accent: '#00d4ff', enemy: '#ff7675' },
            4: { sky: '#1a0505', ground: '#3a1a1a', brick: '#5a2a2a', accent: '#ff4757', enemy: '#ffa502' },
            5: { sky: '#051005', ground: '#1a4a1a', brick: '#2a6a2a', accent: '#7bed9f', enemy: '#e84393' },
            6: { sky: '#050510', ground: '#2a2a4a', brick: '#3a3a6a', accent: '#70a1ff', enemy: '#fd79a8' },
            7: { sky: '#0a0510', ground: '#3a2a5a', brick: '#5a4a7a', accent: '#a29bfe', enemy: '#fdcb6e' }
        };
    ...
    // Stub methods
    submitCode() { } closeCode() { } skipChallenge() { }
    loop() { this.update(); this.draw(); requestAnimationFrame(this.loopBound); }
}

export function initGame() { if (!gameInstance) gameInstance = new Game(); }
